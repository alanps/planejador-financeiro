...
(function($) {
    ...
    $(document).ready(function(){
        ...
        if($(".page-planejador").length > 0 ) {
            var content = new pageUsers($(".page-planejador"));
        }
    ...
    var pageUsers =  function(el) {
	    ...
	    $(document).ready(function() {
			...
			var carregou = 0;
			var carregouTotal = 6;
			window.ordemObjetivos = "dataAdicionado DESC";
			...
			*/
		    ...
		    function cadastroEntrada(e) {
				...
				var entradaTextoTag = el.find(".entradaTipoValorTag").text();
				...
				if (nParcelas <= 1) {
					var entradaData = toTimestamp(el.find(".entradaData").val().split("/")[1] + '/' + el.find(".entradaData").val().split("/")[0] + '/' + el.find(".entradaData").val().split("/")[2] + ' 00:00:00');
				} else {
					var entradaData = toTimestamp(el.find(".entradaVencimentoParcela").val().split("/")[1] + '/' + el.find(".entradaVencimentoParcela").val().split("/")[0] + '/' + el.find(".entradaVencimentoParcela").val().split("/")[2] + ' 00:00:00');
				}

				var dataVencimentoParcela = toTimestamp(el.find(".entradaVencimentoParcela").val().split("/")[1] + '/' + el.find(".entradaVencimentoParcela").val().split("/")[0] + '/' + el.find(".entradaVencimentoParcela").val().split("/")[2] + ' 00:00:00');
				...
				/*} else if (el.find(".entradaTipoValor2").val() == "" || el.find(".entradaTipoValor2").val() == "Tipo de Valor") {
					...
					$.ajax({
					    url: window.homepath + "ajax/cadastroEntrada.php",
					    method: 'POST',
					    dataType: 'json',
					    data: { tipoValorTag: entradaTextoTag, tipoValor: entradaTexto, valorTotal: money, nParcelas: nParcelas, parcelaAtual: parcelaAtual, valorParcela: moneyParcelas, dataVencimentoParcela: dataVencimentoParcela, entradaData: entradaData },
		    ...
		    function cadastroSaida(e) {
				...
				var saidaTextoTag = el.find(".saidaTipoValorTag").text();
				...
				var saidaData = toTimestamp(el.find(".saidaData").val().split("/")[1] + '/' + el.find(".saidaData").val().split("/")[0] + '/' + el.find(".saidaData").val().split("/")[2] + ' 00:00:00');

				var dataVencimentoParcela = toTimestamp(el.find(".saidaVencimentoParcela").val().split("/")[1] + '/' + el.find(".saidaVencimentoParcela").val().split("/")[0] + '/' + el.find(".saidaVencimentoParcela").val().split("/")[2] + ' 00:00:00');
				...
				/*} else if (el.find(".saidaTipoValor2").val() == "" || el.find(".saidaTipoValor2").val() == "Tipo de Valor") {
					...
					$.ajax({
					    url: window.homepath + "ajax/cadastroSaida.php",
					    method: 'POST',
					    dataType: 'json',
					    data: { tipoValorTag: saidaTextoTag, tipoValor: saidaTexto, valorTotal: money, nParcelas: nParcelas, parcelaAtual: parcelaAtual, valorParcela: moneyParcelas, dataVencimentoParcela: dataVencimentoParcela, saidaData: saidaData },
					    success: function (data) {
							console.log("---------------------------------");
		    ...
		    function cadastroObjetivo(e) {
				...
				var objetivoTextoTag = el.find(".objetivoTipoValorTag").text();
				...
				if (!el.find(".objetivoTipoValor").hasClass("objetivoTipoValor2")) {
					...
					el.find(".loading.objetivo_loading").hide();
				} else if (el.find(".objetivoTipoValorTag").text() != "") {
					$.ajax({
		    ...
		    function lerPerfil() {
			    $.ajax({
				    url: window.homepath + "ajax/lerPerfil.php",
				    method: 'POST',
				    dataType: 'json',
				    success: function (data) {
						console.log("---------------------------------");
				    	...
				    	console.log(data);
            			el.find('.box_perfil .nome').text(data.data["0"].nome);
            			el.find('.box_perfil .nome').attr("data-title", data.data["0"].nome);
            			el.find('.box_perfil .profissao').text(data.data["0"].profissao);
            			el.find('.box_perfil .profissao').attr("data-title", data.data["0"].profissao);
            			if (data.data["0"].foto != null) {
	            			el.find('.box_perfil .foto').css("background-image", "url('" + window.homepath + data.data["0"].foto + "')");
	            			el.find('.box_perfil .foto').html("");
	            		}

            			var date = toDate(data.data["0"].dataNascimento);
		    ...
		    function lerUltimas() {
			    ...
			    $.ajax({
				    url: window.homepath + "ajax/lerUltimas.php",
				    method: 'POST',
				    dataType: 'json',
				    data: { dataAdd: window.firstTimestamp, dataFim: window.lastTimestamp },
				    success: function (data) {
						console.log("---------------------------------");
		            	...
		            	$.each(data.data, function(i, item) {
		            		if (data.data[i].table1 == "entrada") {
		            			var table1 = "Recebeu (" + data.data[i].tipoValorTag + ")";
				    			var tableUltima = "entradaUltima";
		            		} else if (data.data[i].table1 == "saida") {
		            			var table1 = "Gastou (" + data.data[i].tipoValorTag + ")";
				    			var tableUltima = "saidaUltima";
		            		} else if (data.data[i].table1 == "objetivos") {
		            			if (data.data[i].tipoValorTag == "Compra") {
			            			var table1 = "Objetivo (Compra)";
				    				var tableUltima = "objetivoCompraUltima";
			            		} else if (data.data[i].tipoValorTag == "Venda") {
			            			var table1 = "Objetivo (Venda)";
				    				var tableUltima = "objetivoVendaUltima";
			            		}
		            		}

	                		var dataUltima = toDateBR(data.data[i].dataAdicionado, "noTime");
	                		var dataUltimaTitle = toDateBR(data.data[i].dataAdicionado, "yes");

				    		el.find(".ultimasNotificacoes").append("<div class='itemUltima'><span class='dataUltima tooltip' data-title='" + dataUltimaTitle + "'>" + dataUltima + "</span><span class='recebeuUltima " + tableUltima + "'>" + table1 + "</span><span class='tipoUltima'>" + data.data[i].tipoValor + "</span> <span class='valorUltima'>(<span class='valorFormatUltima'>" + data.data[i].valorTotal + "</span>)</span></div>");
		    ...
		    function lerEntradaESaida(pagina, ordenacao = "padrao", dataAdd, dataFim) {
    			...
    			el.find(".inANDoutTotalResultados").promise().done(function(){
				    $.ajax({
					    url: window.homepath + "ajax/lerEntradaESaida.php",
					    method: 'POST',
					    dataType: 'json',
						data: { pagina: pagina, items: window.itemsPageEntradaESaida, ordenacao: ordenacao, dataAdd: dataAdd, dataFim: dataFim },
					    success: function (data) {
							console.log("---------------------------------");
			            	...
			            	$.each(data.data, function(i, item) {
	                			...
	                			if (data.data[i].nParcelas > 1) {
									var totalRes = data.data[i].valorParcela * data.data[i].nParcelas;
								} else {
									var totalRes = data.data[i].valorTotal;
								}
	                			...
	                			template.find( ".dadosTipo" ).html("<span class='tag'>" + data.data[i].tipoValorTag + "</span> <span class='valor'>" + data.data[i].tipoValor + "</span>");
	                			template.find( ".dadosParcelas" ).html(data.data[i].nParcelas);


	                			template.find( ".dadosValor" ).html(data.data[i].valorParcela);
			            		...
			            		if (data.data[i].table1 == "entrada") {
		            				var dateEntrada = toDateBR(data.data[i].dataEntrada, "noTime");
		                			template.find( ".dadosData" ).html(dateEntrada);
		                			var dateEntradaTooltip = toDateBR(data.data[i].dataEntrada, "noTime");
		                			template.find( ".dadosData" ).attr("data-title", dateEntradaTooltip);
								} else if (data.data[i].table1 == "saida") {
		            				var dateSaida = toDateBR(data.data[i].dataEntrada, "noTime");
		                			template.find( ".dadosData" ).html(dateSaida);
		                			var dateSaidaTooltip = toDateBR(data.data[i].dataEntrada, "noTime");
		                			template.find( ".dadosData" ).attr("data-title", dateSaidaTooltip);
								}

	                			var dateAdicionado = toDateBR(data.data[i].dataAdicionado, "noTime");
	                			template.find( ".dadosCriadoEm" ).html(dateAdicionado);
	                			var dateAdicionadoTooltip = toDateBR(data.data[i].dataAdicionado, "yes");
	                			template.find( ".dadosCriadoEm" ).attr("data-title", dateAdicionadoTooltip);

	                			if (data.data[i].dataVencimentoParcela != null && data.data[i].dataVencimentoParcela != "null"  && data.data[i].dataVencimentoParcela != undefined && data.data[i].dataVencimentoParcela != "undefined" && data.data[i].dataVencimentoParcela != 0 && data.data[i].dataVencimentoParcela != "0") {
									if (data.data[i].dataVencimentoParcela < Math.floor(Date.now() / 1000)) {
										var parcelaAtualDate2 = new Date(data.data[i].dataVencimentoParcela*1000);
										...
										var parcelaAtualDate = new Date(data.data[i].dataAdicionado*1000);
									} else {
		                				var parcelaAtualDate2 = toDateBR(data.data[i].dataVencimentoParcela);
										var parcelaAtualDate = new Date(data.data[i].dataAdicionado*1000);
									}
								} else if (data.data[i].dataVencimentoParcela < Math.floor(Date.now() / 1000) && data.data[i].dataVencimentoParcela != null && data.data[i].dataVencimentoParcela != "null"  && data.data[i].dataVencimentoParcela != undefined && data.data[i].dataVencimentoParcela != "undefined" && data.data[i].dataVencimentoParcela != 0 && data.data[i].dataVencimentoParcela != "0") {
									if (data.data[i].dataAdicionado < Math.floor(Date.now() / 1000)) {
										var parcelaAtualDate2 = new Date(data.data[i].dataVencimentoParcela*1000);
										...
										var parcelaAtualDate2 = new Date(data.data[i].dataAdicionado*1000);
										console.log(parcelaAtualDate2);
										var parcelaAtualDate2Day = ('0' + parcelaAtualDate2.getDate()).slice(-2);
										var parcelaAtualDate2Month = ('0' + eval(Number(parcelaAtualDate2.getMonth())+2)).slice(-2);
										var parcelaAtualDate2Year = parcelaAtualDate2.getFullYear();
	                			...
	                			var parcelaConta = eval(Number(data.data[i].parcelaAtual) + Number(parcelaAtual));
	                			if (data.data[i].parcelaAtual <= 1) {
		                			if (data.data[i].nParcelas > 1) {
	                				...
	                				template.find( ".dadosAtual" ).html(data.data[i].parcelaAtual);
								}

								if (!template.hasClass("semParcelas") && parcelaConta-1 >= data.data[i].nParcelas && data.data[i].nParcelas > 1) {
							    	template.addClass("vencido");
	                				template.find( ".dadosAtualTd" ).addClass("tooltip").attr("data-title", "Parcelas já finalizadas!");
	                				template.find( ".dadosAtual" ).html(data.data[i].nParcelas);
								} else if (!template.hasClass("semParcelas") && parcelaConta-1 < data.data[i].nParcelas && data.data[i].nParcelas > 1) {
		    ...
		    function lerTotaisEntradaESaida() {
    			...
    			el.find(".totalDiv").promise().done(function(){
				    $.ajax({
					    ...
					    method: 'POST',
					    dataType: 'json',
						data: { pagina: window.pageEntradaESaida, items: window.itemsPageEntradaESaida, ordenacao: window.ordemEntradaESaida, dataAdd: window.firstTimestamp, dataFim: window.lastTimestamp },
					    success: function (data) {
							console.log("---------------------------------");
					    	...
					    	if (data.success == "true") {
						    	el.find(".totalDivTotal .totalDiv_valor").html(data.totalValoresEntrada);
					    	} else {
					    	...
					    	if (data.success == "true") {
						    	el.find(".totalDivGasto .totalDiv_valor").html(data.totalValoresSaida);
					    	} else {
			            	...
			            	//TOTAIS
							var disponivel = data.totalValoresEntrada - data.totalValoresSaida;
		    ...
		    $(document).on("click", ".objetivosPrazo",function(e){
		    	...
		    	if (el.find(".objetivosPrazo").hasClass("objetivosPrazoDESC")) {
			    	window.ordemObjetivos = "dataPrevista DESC";
			    	...
			    	window.ordemObjetivos = "dataPrevista ASC";
			    	if (el.find(".objetivosPaginacao span.paginaSelecionada").length >= 1) {
				    	window.pageObjetivos = el.find(".objetivosPaginacao span.paginaSelecionada").text();
				    } else {
				    	window.pageObjetivos = 1;
		    ...
		    $(document).on("click", ".objetivosTipo",function(e){
		    	...
		    	if (el.find(".objetivosTipo").hasClass("objetivosTipoDESC")) {
			    	window.ordemObjetivos = "tipoValor DESC";
			    	if (el.find(".objetivosPaginacao span.paginaSelecionada").length >= 1) {
				    	window.pageObjetivos = el.find(".objetivosPaginacao span.paginaSelecionada").text();
				    } else {
				    	window.pageObjetivos = 1;
			    	...
			    	window.ordemObjetivos = "tipoValor ASC";
			    	if (el.find(".objetivosPaginacao span.paginaSelecionada").length >= 1) {
				    	window.pageObjetivos = el.find(".objetivosPaginacao span.paginaSelecionada").text();
				    } else {
				    	window.pageObjetivos = 1;
		    ...
		    $(document).on("click", ".objetivosTotal",function(e){
		    	...
		    	if (el.find(".objetivosTotal").hasClass("objetivosTotalDESC")) {
			    	window.ordemObjetivos = "valorTotal ASC";
			    	if (el.find(".objetivosPaginacao span.paginaSelecionada").length >= 1) {
				    	window.pageObjetivos = el.find(".objetivosPaginacao span.paginaSelecionada").text();
				    } else {
				    	window.pageObjetivos = 1;
			    	...
			    	window.ordemObjetivos = "valorTotal DESC";
			    	if (el.find(".objetivosPaginacao span.paginaSelecionada").length >= 1) {
				    	window.pageObjetivos = el.find(".objetivosPaginacao span.paginaSelecionada").text();
				    } else {
				    	window.pageObjetivos = 1;
		    ...
		    function lerObjetivos(pagina, ordenacao = "padrao") {
    			...
    			el.find(".objetivosTotalResultados").promise().done(function(){
				    $.ajax({
					    url: window.homepath + "ajax/lerObjetivos.php",
					    method: 'POST',
					    dataType: 'json',
						data: { pagina: pagina, ordenacao: ordenacao },
					    success: function (data) {
							console.log("---------------------------------");
			            	...
			            	$.each(data.data, function(i, item) {
	                			...
	                			template.find( ".objetivosDadosTipo" ).html("<span class='tag'>" + data.data[i].tipoValorTag + "</span> <span class='valor'>" + data.data[i].tipoValor + "</span>");
	                			template.find( ".objetivosDadosTotal" ).html(data.data[i].valorTotal);
								template.find( ".objetivosDadosTotal" ).priceFormat({
								    prefix: 'R$ ',
								    centsSeparator: ',',
								    thousandsSeparator: '.'
								...
								var dataPrevisao = new Date(toDate(data.data[i].dataPrevista));
								var dataAtual = new Date();
								var dataConta = Math.abs(dataAtual.getTime() - dataPrevisao.getTime());
								var faltam = Math.ceil(dataConta / (1000 * 3600 * 24));
	                			template.find( ".objetivosDadosPrevisao" ).html(faltam + " dias");
	                			template.find( ".objetivosDadosPrevisao" ).attr("data-title", toDateBR(data.data[i].dataPrevista));
	                			template.find( ".objetivosDadosPrevisao" ).attr("date", toDateBR(data.data[i].dataPrevista));
	                			
	                			var now = Math.round(+new Date()/1000);
	                			if (data.data[i].dataPrevista < now) {
